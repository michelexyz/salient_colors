...
data$knows_football <- as.factor(data$knows_football)
...
print(or)
```
The calculated OR value of `r or[1]` means that odds increase by `r or[1]-1`% when the majority team color is red. The 95% confidence interval for the OR is `r or[1:2]`, and because it doesn't include the value of 1 it means that the effect is significant, as previously verified by the McMnemar test.
...
```{r}
summary(glm(correct_response ~ gender, data = data, family = binomial))
```

```{r}
summary(glm(correct_response ~ age, data = data, family = binomial))
```
...
glmm_model1 <- glmer(correct_response ~ 1 + expected_side + (1 | subject_id) + (1 | scene_id),
                    data = data, 
                    family = binomial)
...
# Conclusion
All statistical tests have shown that when the majority team colour is red, subject are more likely to identify the majority team. 
